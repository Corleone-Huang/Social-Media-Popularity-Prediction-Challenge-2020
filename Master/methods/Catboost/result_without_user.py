...
train_feature_pklpath = "../../features/extracted_features/train+label_no_user_305613.pkl"
test_feature_pklpath = "../../features/extracted_features/test+label_no_user_180581.pkl"
...
def train_catboost(X_train,y_train):
    ...
    bst = catboost.train(train_data, params, iterations=iterations,
                         verbose=30, plot=False,save_snapshot=True,snapshot_file="no_user",snapshot_interval=600)
...
def write_log(path="/home/wkai/SMP/log"):
    filename="log_no_user"+"_".join([str(i) for i in time.localtime(time.time())[:5]])+".txt"
...
def write_submission(path="./submission", pids=None, preds=None):
    ...
    tail = "_".join(tail)
    filepath = os.path.join(path, "submission_without_user"+tail+".json")
    with open(filepath, "w") as file:
        json.dump(submission, file)


def main(args):
    log["used_feature"]=args.feature+["mlp"]
    ...
    df_important.to_csv("importance_no_user.csv",index=False)
    # print(df_important)
    print("top 30:")
