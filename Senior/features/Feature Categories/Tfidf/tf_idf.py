...
def parse_args():
    ...
    parser.add_argument('--data_path', default='/home/cx/SMP/data/train/train_tags.json', dest='data_path',
                        type=str, help='文本json数据的路径')
    parser.add_argument('--save_title', default='/home/cx/SMP/data/preprocess/tfidf_tags_305613.csv',
                        dest='save_title',
                        help='存放提取的title特征的路径', type=str)
    parser.add_argument('--save_tags', default='/home/cx/SMP/data/preprocess/tfidf_tags_305613.csv',
                        dest='save_tags',
...
df_data['tags']=df_data['tags'].apply(lambda sen:" ".join(x for x in sen if x not in stop))
df_data['title']=df_data['title'].apply(lambda sen:" ".join(x for x in sen if x not in stop))
print('请稍侯......')
#词性还原
wordnet_lemmatizer = WordNetLemmatizer()
df_data['tags'] = df_data['tags'].apply(lambda x:" ".join([wordnet_lemmatizer.lemmatize(x) for x in x.split() ]))
df_data['title'] = df_data['title'].apply(lambda x:" ".join([wordnet_lemmatizer.lemmatize(x) for x in x.split() ]))
...
file_writer_title.writerow(["pid", "uid"]+["title_feature_"+str(i+1) for i in range(4096)])
file_writer_tags.writerow(["pid", "uid"]+["tags_feature_"+str(i+1) for i in range(4096)])
